...
def calcSize(inputDataArray):
    ...
    totalUsedSpace = getDirSize(folderStructure)

    print('TotalUsedDisk = ' + str(totalUsedSpace))

    FreeSpace = (70000000 - totalUsedSpace)

    print('Free Space is : ' + str(FreeSpace))

    TargetSize = 30000000 - FreeSpace

    print('Target size to delete is :' + str(TargetSize))

    SmallestFileSize = 70000000

    for item in total:
        if (item >= TargetSize):
            if(item < SmallestFileSize):
                SmallestFileSize = item
...
def calculateSizeTotals(folderStructure):
    ...
    if(folderStructure.filetype == 'Directory'):
        ...
        totals.append(getDirSize(folderStructure))
        for item in folderStructure.children:
            totals += calculateSizeTotals(item)
    # else:
    #     print(preString + folderStructure.name + ' - ' + str(folderStructure.filesize))
